...
export default class room4 extends Phaser.Scene {
    ...
    preload() {
    // Step 1, load JSON
    this.load.tilemapTiledJSON("room4","assets/empty-room4.json");
    ...
    create() {
        ...
        let map = this.make.tilemap({ key: "room4" });
  ...
  this.dog10 = this.physics.add.sprite(270, 600, 'dog10').play("dog");

  this.time.addEvent({
    delay: 1000,
    callback: this.moveLeftRight10,
    callbackScope: this,
    loop: false,
  });
    ...
    this.physics.add.overlap(this.player, this.dog10, this.enemyOverlap, null, this);
    }

    moveLeftRight10() {
      console.log("moveLeftRight10");
      this.tweens.timeline({
        ...
        tweens: [
          ...
          {
            x: 308, //must same with add sprite x
          },
        ],
      });
    }
